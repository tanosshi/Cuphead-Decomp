...
public class BatLevelBouncer : AbstractCollidableObject
{
	...
	protected IEnumerator move_cr()
	{
		...
		while (true)
		{
			base.transform.position += velocity * properties.mainBounceSpeed * CupheadTime.FixedDelta;
			yield return null;
		}
	}
	...
	protected override void OnCollisionCeiling(GameObject hit, CollisionPhase phase)
	{
		...
		if (phase == CollisionPhase.Enter && lastSideHit != SideHit.Top)
		{
	...
	protected IEnumerator change_direction_cr(Vector3 collisionPoint)
	{
		...
		counter++;
		if ((float)counter >= properties.breakCounter && !isPink)
		{
			SpawnPink();
			Die();
		}
	...
	{
		BatLevelPinkCore batLevelPinkCore = Object.Instantiate(pinkPrefab);
		batLevelPinkCore.Init(properties, base.transform.position, angle);
	}
