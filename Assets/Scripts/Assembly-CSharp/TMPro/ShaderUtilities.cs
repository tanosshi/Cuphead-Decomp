...
namespace TMPro
{
	public static class ShaderUtilities
	{
		...
		public static void UpdateShaderRatios(Material mat, bool isBold)
		{
			...
			float num8 = (isBold ? (mat.GetFloat(ID_WeightBold) * 2f / num4) : (mat.GetFloat(ID_WeightNormal) * 2f / num4));
			float num9 = Mathf.Max(1f, num8 + num5 + num6 + num7);
			num = ((!flag) ? 1f : ((num4 - m_clamp) / (num4 * num9)));
			...
			if (mat.HasProperty(ID_GlowOffset))
			{
				...
				num2 = ((!flag) ? 1f : (Mathf.Max(0f, num4 - m_clamp - num12) / (num4 * num9)));
			...
			if (mat.HasProperty(ID_UnderlayOffsetX))
			{
				...
				num3 = ((!flag) ? 1f : (Mathf.Max(0f, num4 - m_clamp - num15) / (num4 * num9)));
